# Reglas y Preferencias del Proyecto Clinova

## 1. Arquitectura y Despliegue en VPS y Hosting
- **EL VPS NUNCA ESTÁ CONECTADO A NINGÚN REPOSITORIO GIT**:
  - No sugerir `git pull` en el VPS bajo ninguna circunstancia.
  - El backend se actualiza y compila localmente generando el `.jar` (`clinova-backend.jar`).
  - El `.jar` se transfiere directamente al VPS (vía SCP / SFTP) a `/opt/clinova/clinova-backend.jar` y se reinicia el contenedor Docker:
    `sudo docker restart clinova-backend`
  - El backend SIEMPRE debe mantenerse con commit y push en su repositorio GitHub (`https://github.com/kevihxd/CLINOVA-0.1.git`) para control de versiones.
...
- **EL FRONTEND SE DESPLIEGA DIRECTAMENTE POR HOSTINGER**:
  ...
  - También se mantiene generado `dist.tar.gz` en el Escritorio como respaldo si se requiere.
