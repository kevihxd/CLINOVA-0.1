...
public class VacunacionController {
    ...
    public ResponseEntity<Vacuna> addVacuna(@RequestBody Vacuna vacuna) {
        return ResponseEntity.ok(vacunacionService.crearVacuna(vacuna));
    }

    @PutMapping("/catalogo/{id}")
    public ResponseEntity<Vacuna> editarVacuna(@PathVariable Long id, @RequestBody Vacuna vacuna) {
        return ResponseEntity.ok(vacunacionService.editarVacuna(id, vacuna));
    }
